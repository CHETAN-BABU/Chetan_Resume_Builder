        ...
        with self.studio.lock:
            ...
            if field or project or font:
                if not job_id or revision is None:
                    ...
                    try:
                        if field:
                            draft = self.studio.save(job_id, revision, fields={('ResumeSummary' if field[1].lower() == 'summary' else 'CoreSkills'): field[2]})
                        elif project:
                            ...
                            if size is not None and not 10 <= size <= 12:
                                raise ValueError('Body font must stay between 10 and 12pt for readability')
