...
class BatchQualityTests(unittest.TestCase):
    ...
    def build_passed_artifact(
        ...
        shutil.copy2(ROOT / "templates/resume-base.tex", source_path)
        jd_path = artifact_dir / "job-description.md"
        jd_path.write_text(
            f"# {company} BI Analyst\n\nPower BI, SQL, reporting, and validation.\n",
            encoding="utf-8",
        )
        (artifact_dir / "evaluation.md").write_text(
            "# Evaluation\n\nEligible BI Analyst role with supported requirements.\n",
            encoding="utf-8",
        )
        (artifact_dir / "company-research.md").write_text(
            "# Company research\n\nCurrent reporting modernisation is the selected problem.\n",
            encoding="utf-8",
        )
            ...
            "requirements": [
                {
                    "id": "R01",
                    "priority": "required",
                    "text": "Power BI reporting and validation",
                    "evidence_ids": ["PROJ-AZ-RECON"],
                    "strength": "strong",
                }
            ],
            "company_problem": {
                "statement": (
                    f"{company} needs reliable reporting and validation for business decisions."
                ),
                "source_url": "https://www.microsoft.com/en-us/power-platform/products/power-bi",
