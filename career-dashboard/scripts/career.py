...
class Workspace(Tracking):
    ...
    def export_tracking(self):
        ...
        lines = [
            "# Application tracker — Chetan Babu M",
            "",
    ...
    def screen(self, job):
        ...
        issues = []
        if re.search(r"\b(senior|sr\.?|lead|principal|director|manager|head)\b", title):
            issues.append(
                "Seniority needs evidence beyond the current early-career profile."
            )
        if re.search(
            r"\b(software|full.?stack|devops|mlops|hardware|embedded|firmware)\b", title
        ):
            issues.append(
                "This engineering track is outside the current professional profile."
            )
        if re.search(r"\b(?:ml|machine learning|ai|genai) engineer\b", title):
            issues.append(
                "Professional AI/ML engineering is unsupported; confirm whether the role explicitly accepts candidate-project evidence."
            )
        if re.search(r"\bdata engineer\b", title):
            issues.append(
                "Check for production pipeline/platform ownership requirements."
            )
        if not re.search(
            r"ireland|dublin|cork|limerick|galway|waterford|kilkenny|athlone|tralee|kerry|donegal|sligo|wexford|tipperary|kildare|meath|wicklow|clare|mayo|laois|louth|cavan|monaghan|roscommon|longford|leitrim|offaly|carlow|westmeath",
            location,
        ):
            issues.append("Ireland eligibility is not established by this location.")
        return {
            "decision": "review_required",
            "concerns": issues,
            "note": "A title/keyword screen is not an eligibility decision. Review the full requirements and work-permission conditions.",
        }
    ...
    def prepare(self, job_id, project_id=None):
        ...
        out = self.root / "output/applications"
    ...
    def current_folder(self, job_id):
        ...
        return safe_child(
            self.root / "output", str(Path(job["folder"]).relative_to("output"))
        )
    ...
    def compile_preview(self, job_id):
        ...
        with tempfile.TemporaryDirectory(prefix="chetan-compile-") as temp:
            ...
            if report["page_count"] != 2:
                raise ValueError(
                    f"Expected two pages; found {report['page_count']}. Edit the source before release."
                )
    ...
    def validate(self, job_id):
        ...
        result = subprocess.run(
            [
                sys.executable,
                str(self.root / "scripts/validate_resume.py"),
        ...
        for p in sorted((self.root / "output").glob("**/*.pdf")):
            ...
            if qa_path.exists():
                try:
                    ...
                    pass
            source = (
                self.root / "templates/resume-base.tex"
            ...
            stale = (
                ...
                != hashlib.sha256(
                    (self.root / "context/evidence.yml").read_bytes()
                ).hexdigest()
                or qa.get("profile_sha256")
                != hashlib.sha256(
                    (self.root / "config/profile.yml").read_bytes()
                ).hexdigest()
            )
            ...
            items.append(
                {
                    "path": str(p.relative_to(self.root / "output")),
                    "name": p.name,
                    "status": (
                        "REVIEW_REQUIRED"
                        if stale
...
def main():
    ...
    if args.command == "status":
        ...
        result = w.add_job(**json.loads(args.file.read_text()))
