...
EXPECTED_PREVIEWS = ("page-01.png", "page-02.png")
VISIBLE_INVARIANTS = (
    "Chetan Babu M",
...
def inspect_published_pdf(
    ...
    with tempfile.TemporaryDirectory(prefix="batch-published-pdf-") as temporary:
        ...
        if rendered_files != list(EXPECTED_PREVIEWS):
            failures.append(
                "Published PDF did not render exactly page-01.png and page-02.png"
            )
        for filename in EXPECTED_PREVIEWS:
        ...
        if page_count != 2:
            failures.append(f"Published PDF must have exactly 2 pages; found {page_count}")
        for page in pages:
            if not isinstance(page, dict):
                continue
            width = float(page.get("width_points", 0))
            height = float(page.get("height_points", 0))
            if abs(width - 595.3) > 3 or abs(height - 841.9) > 3:
                failures.append(
                    f"Published PDF page {page.get('number')} is not A4 "
        ...
        if not isinstance(metadata, dict):
            metadata = {}
        if not (
            metadata.get("author") == "Chetan Babu M"
            and str(metadata.get("title", "")).startswith("Chetan Babu M")
        ):
            failures.append("Published PDF metadata author/title does not match Chetan")
...
def revalidate_current_source(
    ...
    with tempfile.TemporaryDirectory(prefix="batch-source-recheck-") as temporary:
        ...
        if not (
            ...
            and fresh_qa.get("page_count") == 2
            and fresh_qa.get("published_pdf") is True
        ):
            failures.append("Fresh QA did not satisfy the automated two-page contract")
...
def audit_passed_artifact(
    ...
    if manifest_page_count is not None and manifest_page_count != 2:
        failures.append("Manifest page_count does not match the two-page artifact")
    ...
    report["page_count"] = 2 if published_text and not any(
        "exactly 2 pages" in message for message in failures
    ) else None
...
def validate_manifest(manifest_path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        evidence = load_yaml(ROOT / "context/evidence.yml")
    except (RuntimeError, json.JSONDecodeError) as exc:
