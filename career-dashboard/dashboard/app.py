"""Chetan's local dashboard. Bind to loopback through run.py."""
...
def create_app(root=ROOT):
    ...
    async def local_only(request: Request,call_next):
        if request.url.hostname not in {'127.0.0.1','localhost','::1'}:
            return JSONResponse({'detail':'Use the local dashboard address'},status_code=403)
        ...
        response.headers['Content-Security-Policy']="default-src 'self'; style-src 'self'; script-src 'self'; img-src 'self' data:; frame-src 'self'; object-src 'none'; base-uri 'none'; frame-ancestors 'none'"
        return response
    ...
    def overview():
        ...
        historical=json.loads((root/'data/historical-packs.json').read_text()) if (root/'data/historical-packs.json').exists() else []
        return {'candidate':workspace.profile()['candidate'],'identity':workspace.profile()['professional_identity'],'revision':evidence['candidate_revision'],'counts':{'claims':len(evidence['claims']),'projects':sum(bool(bool(p.get('resume_content'))) for p in evidence['projects']),'saved_jobs':len(jobs),'applied':sum(bool(j['application_date']) for j in jobs),'historical_packs':len(historical)},'artifacts':workspace.artifacts(),'questions':(root/'context/QUESTIONS-FOR-YOU.md').read_text(),'historical':historical}
    @app.get('/api/profile')
    def profile(): return {'profile':workspace.profile(),'evidence':workspace.evidence(),'notes':(root/'context/UPDATES.md').read_text() if (root/'context/UPDATES.md').exists() else ''}
    @app.put('/api/profile/notes')
    def save_notes(data: NotesInput):
        return workspace.save_profile_notes(data.text)
    @app.get('/api/activity')
    def activity(): return workspace.activity()
    @app.get('/api/search-runs')
    def searches(): return {'today':today(), 'runs':workspace.search_runs()}
    @app.post('/api/search-runs/{date}')
    def start_search(date: str): return workspace.start_search(date)
    @app.put('/api/search-runs/{date}')
    def update_search(date: str, data: NotesInput): return workspace.update_search(date, data.text)
    @app.post('/api/search-runs/{date}/jobs/{job_id}')
    def attach_search_job(date: str, job_id: str): return workspace.track_search_job(job_id,date)
    @app.get('/api/projects')
    def projects(q: str=''): return workspace.rank_projects(q)
    @app.get('/api/jobs')
    def jobs(): return workspace.jobs()
    @app.post('/api/jobs',status_code=201)
    def add_job(data: JobInput): return workspace.add_job(**data.model_dump())
    @app.get('/api/jobs/{job_id}')
    def job(job_id: str):
        value=workspace.get_job(job_id)
        return {'job':value,'screen':workspace.screen(value),'projects':workspace.rank_projects(value['title']+' '+value['description'])}
    ...
    def preview(job_id: str):
        with job_lock(job_id): return workspace.compile_preview(job_id)
    @app.post('/api/jobs/{job_id}/validate')
    def validate(job_id: str):
        with job_lock(job_id): return workspace.validate(job_id)
    @app.get('/api/portals')
    def portals():
        from career import read_yaml
        return read_yaml(root/'config/portals.yml')
    @app.get('/api/files/{relative:path}')
    def output_file(relative: str):
        try: path=safe_child(root/'output',relative)
