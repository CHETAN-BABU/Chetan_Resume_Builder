...
def create_app(root=ROOT):
    ...
    @app.get("/api/health")
    def health():
        return {"app": "chetan-career-workspace", "version": "2.0.0", "root": str(root)}
    ...
    def overview():
        ...
        return {
            ...
            "questions": (root / "context/QUESTIONS-FOR-YOU.md").read_text(),
    ...
    def profile():
        return {
            ...
            "evidence": workspace.evidence(),
            "notes": (
                (root / "context/UPDATES.md").read_text()
                if (root / "context/UPDATES.md").exists()
                else ""
            ),
        }
    ...
    def prepare(job_id: str, data: PrepareInput):
        if service.profile_dirty():
            raise ValueError(
                "Profile edits are saved. Reconcile the resume evidence registry before preparing a new draft so outdated facts cannot be used."
            )
    ...
    def output_file(relative: str):
        try:
            path = safe_child(root / "output", relative)
        except ValueError:
